...
func CmdList(c *gin.Context) {
	...
	gitList, _ := common.DbMain.Client.QuickQuery(`tbl_variable`, `*`, nil).All()
...
func CmdAdd(c *gin.Context) {
	...
	if cast.ToInt(dataMap[`id`]) == 0 {
		...
		_, createErr := common.DbMain.Client.QuickCreate(`tbl_variable`, updateData).Exec()
		if createErr != nil {
...
func CmdDelete(c *gin.Context) {
	...
	if cast.ToInt(dataMap[`id`]) == 0 {
		...
		_, _ = common.DbMain.Client.QuickDelete(`tbl_variable`, map[string]any{
